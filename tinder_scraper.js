...
Java.perform(function () {
    let DeviceIdFactoryImpl = Java.use("com.tinder.library.auth.internal.usecase.DeviceIdFactoryImpl");
DeviceIdFactoryImpl["getDeviceId"].implementation = function () {
    let result = this["getDeviceId"]();
    devicePersist = result ;
    return result;
}; 
});
Java.perform(function () {
    console.log("[*] Hooking ProtoResponseBodyConverter.convert...");
    var ProtoResponseBodyConverter = Java.use("retrofit2.converter.protobuf.ProtoResponseBodyConverter");
    ProtoResponseBodyConverter.convert.overload("java.lang.Object").implementation = function (arg) {
        var result = this.convert(arg);
        if (result && result.toString().toLowerCase().includes("refresh_token") && result.toString().toLowerCase().includes("auth_token_ttl") ) {
            console.log(result.toString());
            console.log("\nDevice Persist:"+devicePersist+ "\n");
        } 
        return result; 
    };
});
